...
//                CGContextRestoreGState(context);
                ...
                [[NSString stringWithFormat:@"%ld-%@",(long)[components day],[self getMonthNameByNumber:[components month]]] drawAtPoint:CGPointMake(i-10, kTopMargin + self.chartHeight + 10) withFont:kStatesticFont];
                CGContextStrokePath(context);
            }
            ...
            [[NSString stringWithFormat:@"%ld-%@",(long)[components day],[self getMonthNameByNumber:[components month]]] drawAtPoint:CGPointMake(kTopMargin + self.chartWidth-10, kTopMargin + self.chartHeight + 10) withFont:kStatesticFont];
...
//                    CGContextAddLineToPoint(context, self.curPoint.x, self.curPoint.y);
                    ...
                    // *** Calculate curve between two points for smoothness ***
                    CGPoint midPoint = midPointForPoints(self.prevPoint, self.curPoint);
                    CGPoint cp1 = controlPointForPoints(midPoint, self.prevPoint);
                    CGContextAddQuadCurveToPoint(context, cp1.x, cp1.y, midPoint.x, midPoint.y);
                    CGPoint cp2 = controlPointForPoints(midPoint, self.curPoint);
                    CGContextAddQuadCurveToPoint(context, cp2.x, cp2.y, self.curPoint.x, self.curPoint.y);
